...
\packageVersionDate{3 October 2011}

\title{\sbmlpkg: A \latex Class for \mbox{SBML Specification Documents}}
...
The \sbmlpkg class for \latex provides a standard format for authoring SBML
Level~3 \emph{package specification} documents.  In the paragraphs below, I
explain how to use this class file.  I assume readers are already familiar
with \latex.  (There are many tutorials available on \latex today, and
readers should have not trouble finding them on the Internet if they need
more basic information than what is provided here.)

This document is itself formatted using \sbmlpkg, with a minor change to
omit some SBML package-specific text normally placed on the front page.
(For example, the front page of this document does not announce it is an
``SBML Level 3 Package Specification'', nor does it provide a package URL
and other similar information.)  In all other respects, ``what you see is
what you get''---this is the appearance of an SBML Level~3 package
specification document when it is formatted with \sbmlpkg.


\subsection{Prerequisites and installation}

\sbmlpkg itself consists of only a single file, \texttt{sbmlpackage.cls}.
It comes with some accompanying documentation (specifically, the file you
are reading, along with the source files to produce it), but the actual
class definition is in the file \texttt{sbmlpackage.cls}.  Its use should
require only a recent and relatively complete installation of \latex2e.  I
developed and tested \sbmlpkg with the TeX Live 2009 distribution on a
Mac~OS~X 10.6.x system.  \tab{where} lists the Internet locations from
where you may obtain \sbmlpkg.
...
\item \emph{Per-document installation}.  This is arguably the simplest
  ...
  \tab{where}), copy the file \texttt{sbmlpackage.cls} to the
  ...
  explained in \sect{usage}), it will find the \texttt{.cls} file in the
...
The class defines many commands, and a full list is given in
\sect{provides}.  The aim of the present section is to provide a summary
...
\sbmlpkg preloads the \latex \texttt{graphicx} and \texttt{xcolor} classes,
giving authors immediate access to the functionality of these extensions
without having to include them manually.  For example, if you had an
illustration in a file named ``\texttt{example.pdf}'', the following
...
The \texttt{graphicx} class is extremely powerful; it offers many options
for controlling the size/scale and other characteristics of graphics files.
Please refer to its documentation for help on how to use it fully.
...
For tables, \sbmlpkg preloads the \texttt{booktabs} class.  It provides
...
\begin{example}
\begin{table}[hb]
  \begin{edtable}{tabular}{ll}                 % From the lineno class; see text, Section 2.2
...
\texttt{hyperref} class, and like the other classes mentioned in this
...
object classes.  They are designed to be as convenient to use as possible,
requiring only one additional character (i.e., the leading backslash
character) to be typed beyond the name of the object class itself.
...
When a package specification document defines new object classes, it is
...
\item[\textbackslash defRef\{\emph{name}\}\{\emph{section}\}] Create a
  ...
  \emph{object}.  The reference appears at the point in the text where the
...
Sometimes it is desirable to write the names of classes without introducing
hyperlinks.  A common situation is when mentioning the names of the object
classes or types in section headings.  In these situations, instead of
using the \cmd{ObjectName} commands, you may use the following commands
provided by \sbmlpkg:
...
In addition to formatting the names of object classes and primitive data
types, \sbmlpkg provides commands for formatting text meant to be literal
examples of XML.  These special commands are provided so in-text
...
\sbmlpkg preloads the \latex class \texttt{lineno} and configures it to
...
\subsection{Notable \latex classes preloaded by \sbmlpkg}
\label{included-classes}

As mentioned above, \sbmlpkg preloads many common \latex classes.  Some are
...
\item \texttt{amssymb}: This class defines a number of symbols and special
...
\item \texttt{booktabs}: Already mentioned in \sect{about-tables}, this
  \latex class defines the commands \cmd{toprule}, \cmd{bottomrule},
  and \cmd{midrule} for produced more attractive and
  professional-looking tables.

\item \texttt{enumitem}: This relatively new class adds facilities for more
...
\item \texttt{hyperref}: Mentioned in \sect{hyperlinks}, this class defines
...
\item \texttt{wasysym}: Similar to \texttt{amssymb}, this \latex class
  defines another set of symbols and special characters.  

\item \texttt{xcolor}: This class defines a large number of color names and
  associated commands.
